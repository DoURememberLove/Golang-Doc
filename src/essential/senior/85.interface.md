...
结构体的组合主要目的是代码复用，其实`Person`是`Man`的一个字段，在实例化后的内存中，`Person`是存在于`Man`内部的，同时，Go 做了字段提升和方法提升，所以以下写法都是可以的：
...
其实就是一个，方法签名完全一致，保留一个即可。但是如果携带不同的参数，就不一样了，Go 不支持重载，所以名称必须不同。
