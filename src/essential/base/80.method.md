...
上述代码运行过后，会发现`myInt`的值依旧是 1，并没有被修改成 2。方法在被调用时，会将接收者的值传入方法中，上例的接收者就是一个值接收者，可以简单的看成一个形参，而修改一个形参的值，并不会对方法外的值造成任何影响（基础类型，如果是引用类型，比如 slice 或者 map，那么由于共享底层数据，是会变化的），那么如果通过指针调用会如何呢？
...
func main() {
    p := &Person{"Tom"}
    p.Say()  // 1.调用方法，Go 在方法调用时会自动取地址、自动解引用，从而符合接收者，反之也一样
	fmt.Println(p.Name)   // 2.结构体调用，等价于(*p).Name

	arr := &[3]int{1,2,3}
	fmt.Println(arr[0])   // 3.数组指针，等价于(*arr)[0]
}
...
可以认为： Go 的方法在编译阶段会转换成普通函数，只不过编译器帮你隐藏了第一个参数（`Receiver`）。从语言语义上看，方法并不是函数；但编译器实现时，方法基本都会转换成带`Receiver`参数的普通函数，并生成相应的方法信息用于支持`obj.Method()`的调用。比如：
...
这也就解释了上面为什么上面要用指针接收者才能改变值，因为函数传参是值拷贝，函数内改变形参（基本类型）并不会影响外部参数。如果传递的是一个整型，那就拷贝这个整型，如果是一个切片，那就拷贝这个切片，但如果是一个指针，就只需要拷贝这个指针，显然传递一个指针比起传递一个切片所消耗的资源更小，接收者也不例外，值接收者和指针接收者也是同样的道理。在大多数情况下，都推荐使用指针接收者，不过两者并不应该混合使用，要么都用，要么就都不用，看下面一个例子。
