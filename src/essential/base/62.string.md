...
字符串通常直接赋值就够了。所谓“字符串拷贝”大多数时候没有必要，因为字符串是不可变的。拷贝主要不是为了防止修改，而是为了避免小字符串长期引用大块内存。

```go
response := loadHugeResponse()
token := strings.Clone(response[100:132])
```

这样 token 不再依赖整个大字符串，response可以进行回收。
